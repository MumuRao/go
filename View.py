...
def Main():
    ...
    if q:  # 基于搜索条件显示结果
        ...
        cursor = Mongo.coll['Info'].find({}).sort('time', -1).limit(page_size).skip((page - 1) * page_size)
        return render_template('main.html', item=cursor, plugin=plugin, itemcount=cursor.count(), plugin_type=plugin_type)
...
def Task():
    ...
    #number = Mongo.coll['Task'].find({"vul_info.vul_name":"Redis弱口令"}).count() #漏洞数量
    return render_template('task.html', item=cursor)


# 任务列表扫描目标页面
@app.route('/tasktarget')
@logincheck
def TaskTarget():
    ...
    Tarcount = len(target_list)

    return render_template('tasktarget.html', item=target_list, count=Tarcount, plugin_name=plugin_name, id=id)
...
def DelTarget():
    ...
    t_ip, t_port = host.split(":")

    t_ = []
    t_.append(t_ip)
    t_.append(int(t_port))
    
    task_info = Mongo.coll['Task'].find_one({'_id': ObjectId(id)})

    target_list = task_info["target"]
    try:
        target_list.remove(t_)
        Mongo.coll['Task'].update({'_id': ObjectId(id)}, {"$set":{"target": target_list}})
        result = 'success'
    except:
        result = 'fail'
...
def TaskDetail():
    ...
    if len(lastscan) > 0:
        ...
        for _ in cursor:
            data = Mongo.coll['Data'].find_one({"$or":[{"public_ip":_['ip']},{"inner_ip":_['ip']}]})
            if not data:    #判断ip是否有data
                data = {"envi":"None","public_ip":"None","inner_ip":"None","CreateTime(UTC)":"None"}

            result_list.append(
                {'ip': _['ip'], 'port': _['port'], 'info': _['info'], 'vul_level': _['vul_info']['vul_level'],
                'time': _['time'], 'data': data, 'vul_info': _['info']})
...
def DownloadXls():
    ...
    if tid:  # 有任务id
        ...
        for _ in cursor:
            ...
            result_list.append(
                {'ip': _['ip'], 'port': _['port'], 'info': _['info'], 'vul_level': _['vul_info']['vul_level'],
                'time': _['time'], 'vul_name': _['vul_info']['vul_name'], 'instance_name': data['instance_name'], 'envi': data['envi'], 
                'lastscan': taskdate, 'title': title,'hostname': hostname, 'public_ip':data['public_ip']})
        ...
        for t in tasks:
            ...
            for _ in cursor:  # 单任务详情
                ...
                _['lastscan'] = taskdate
                _['instance_name'] = data['instance_name']
                _['envi'] =  data['envi'],
                t_list.append(_)
...
def search_result_xls():
    ...
    if query:
        ...
        cursor = Mongo.coll['Info'].find({}).sort('time', -1)
    title_tup = ('IP', '端口号', '名称', '服务类型', '是否存在风险')
    xls = [title_tup, ]
    for info in cursor:
        result_mongo =  Mongo.coll['Result'].find({'ip':info['ip'], 'port':info['port']})
        if result_mongo.count() == 0 : is_safe = '否'
        else: 
            for i in result_mongo:
                is_safe = i['info']
        data = Mongo.coll['Data'].find_one({"$or":[{'public_ip': info['ip']},{'inner_ip': info['ip']}]})
        if not data:    #判断ip是否有data
            data = {"envi":"None","public_ip":"None","inner_ip":"None","CreateTime(UTC)":"None"}
        item = (
            info.get('ip'), info.get('port'), data.get('instance_name'),
            info.get('server'), is_safe)
        xls.append(item)

...
def request_payload(request, payload, param, postdata=False, time_check=False, jsondata=False):
    def json_data(data, payload, param):
        try:
            data_ = json.loads(data['postdata'])
        except:
            data_ = data
        ...
        if param in data_.keys():
            ...
            if not isinstance(value, int):
                tmp_data[param] = data_[param] + payload
                #print(data_)
                return json.dumps(tmp_data)
            else:
                tmp_data[param] = str(data_[param]) + payload
                return json.dumps(tmp_data)
        else:
            for value in data_.values():
                tmp_value = str(value)
                if isinstance(value, dict):
                    json_data(value, payload, param)    #循环用
                    tmp_data = json.dumps(data_).replace(tmp_value, str(value))
                    return tmp_data
            
    if postdata and jsondata:
        postdata = json_data(request, payload, param)
        res, times = time_requests(request['method'], request['url'], request['headers'], postdata)
    elif postdata and not jsondata:
    ...
    for node in dom.getElementsByTagName('couple'):
        ...
        if couple_id <= config_level:
            ...
            for payload in payloads.splitlines():
                ...
                #增加json请求判断
                if request['headers']["Content-Type"] and "application/json" in request['headers']["Content-Type"]:
                    json_key_list = []
                    json_data = json.loads(request['postdata'])
                    for key in json_key(json_data):
                        response = request_payload(request, payload.strip(), key, postdata=True, jsondata=True)
